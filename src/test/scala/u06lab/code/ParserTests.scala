...
class ParserTests:
  ...
  def parserSN = new BasicParser(Set('a', 'b', 'c')) with ShortenThenN[Char]
  ...
  @Test
  def testShorterThenNParser(): Unit =
    assertTrue(parserSN.parseAll("".toList))
    assertTrue(parserSN.parseAll("ab".toList))
    assertTrue(parserSN.parseAll("abc".toList))
    assertTrue(parserSN.parseAll("aabc".toList))
    assertTrue(parserSN.parseAll("a".toList))
    assertFalse(parserSN.parseAll("aabcdc".toList))
    assertFalse(parserSN.parseAll("aabcdcsadas".toList))
